...
const suporteResponse = await fetch(`${BASE_PATH}/api/suporte/${encodeURIComponent(cotacaoCode)}`, {
                                method: 'POST',
                                headers: { 'Authorization': `Bearer ${token}` },
    ...
    const fetchSuporteData = async () => {
        ...
        try {
            ...
            const BASE_PATH = window.location.pathname.startsWith('/pme_notas') ? '/pme_notas' : '';
            const cotacaoCode = editingQuotation.cotacao.includes(' - ') ? editingQuotation.cotacao.split(' - ')[1] : editingQuotation.cotacao;
            const response = await fetch(`${BASE_PATH}/api/suporte/${encodeURIComponent(cotacaoCode)}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
                const data = await response.json();
    ...
    const getStatusConfig = (status) => {
        ...
        if (normalized === 'renovacao-aparelho') {
            return { label: 'Renovação/Aparelho', className: 'bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100', dotClass: 'bg-teal-500' };
        }
        if (normalized === 'pendente-input-hitss') {
            return { label: 'Pendente Input - Hitss', className: 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100', dotClass: 'bg-amber-500' };
        }
        if (normalized === 'aguardando-bilhete') {
            return { label: 'Aguardando Bilhete', className: 'bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100', dotClass: 'bg-rose-500' };
        }
                                                ...
                                                <div>
                                                    ...
                                                    <div className="space-y-2">
                                                        {suporteData.anexos.map((anexo) => (
                                                            <div key={anexo.id} className="flex items-center bg-slate-50 px-3 py-2 rounded-lg border border-slate-200">
                                                                <div className="flex items-center gap-2">
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 modal-overlay p-4" onClick={(e) => { if (e.target === e.currentTarget) setStatusModal(null); }}>
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl modal-content">
                        ...
                        <div className="space-y-2.5 max-h-[50vh] overflow-y-auto">
                            ...
                            <button onClick={() => handleStatusChange('pendente-input-hitss')} className="w-full flex items-center gap-3 px-4 py-3 bg-amber-50 text-amber-700 border border-amber-200 rounded-xl hover:bg-amber-100 transition-all duration-200 font-semibold text-sm"><span className="w-2.5 h-2.5 rounded-full bg-amber-500"></span>Pendente Input - Hitss</button>
                            ...
                            {statusModal.origem === 'iw_cpc_975_net' && (
                                <>
                                    ...
                                    <button onClick={() => handleStatusChange('renovacao-aparelho')} className="w-full flex items-center gap-3 px-4 py-3 bg-teal-50 text-teal-700 border border-teal-200 rounded-xl hover:bg-teal-100 transition-all duration-200 font-semibold text-sm"><span className="w-2.5 h-2.5 rounded-full bg-teal-500"></span>Renovação/Aparelho</button>
